...
from keyboard.inline.cool_nocool import cool
from loader import dp, bot
from states.state_for_channel import Get1


# Ответ на кнопку


@dp.callback_query_handler(text="offermem")
async def offer_post(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("Отправьте фото, видео, гифку или альбом мема😁")
    await Get1.EnterMedia.set()


# Кнопка готово


@dp.message_handler(Users(), state=Get1.Done, text="Готово✅")
async def get_photo_mem(message: types.Message, state: FSMContext):
    await message.answer("Отправь боту текст мема🗒")
    await Get1.EnterText.set()


# Получение медиа для мема


@dp.message_handler(Users(), state=Get1.EnterMedia, content_types=types.ContentType.ANY)
async def get_photo_mem(message: types.Message, state: FSMContext):
    ...
    await message.answer('Отлично нажми кнопку "готово"', reply_markup=done_sending)
    await Get1.Done.set()
...
@dp.message_handler(Users(), state=Get1.EnterText)
async def get_text_for_post(message: types.Message, state: FSMContext):
    await message.answer("Текст успешно добавлен✅\nТеперь подтверди публикцию, после этого"
                         " мем будет отправлен на модерацию")
    await state.update_data(text=message.html_text, mention=message.from_user.mention,
                            notify=message.chat.id)
    await message.answer("Вы точно хотите опубликовать мем?🤨", reply_markup=confirm)
    await Get1.Confirm.set()


# Ответ на уточнение


@dp.callback_query_handler(state=Get1.Confirm, text="Yes")
...
@dp.callback_query_handler(state=Get1.Confirm, text="No")
async def poster(call: CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(state=Get1.Confirm)
...
async def approve(call: CallbackQuery, state: FSMContext):
    ...
    await message.send_copy(chat_id=channels[0], reply_markup=cool)
